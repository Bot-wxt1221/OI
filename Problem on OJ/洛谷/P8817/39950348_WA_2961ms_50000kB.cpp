...
unsigned long long dp[4][2505];
long long choose[4][2505];
long long vised[2505];
queue<long long>in;
unsigned long long Map[2505][2505];
unsigned long long score[2505];
		...
		for(long long j=2;j<=n;j++){
			...
			for(int ia=1;ia<=3;ia++){
                for(int ja=1;ja<=3;ja++){
                    if(choose[ia][i]!=choose[ja][j]&&choose[ia][i]!=j&&choose[ia][i]!=i&&choose[ja][j]!=i&&choose[ja][j]!=j&&choose[ja][j]!=0&&choose[ia][i]!=0){
