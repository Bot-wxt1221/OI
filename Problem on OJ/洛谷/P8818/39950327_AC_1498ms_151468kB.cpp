...
signed main(){
    ...
    for(int i=1;i<=n;i++){
        ...
        if(mina.st[i][0]<0){
            ...
            dmin.st[i][0]=0x3f3f3f3f3f3f3f3fll;
        }
        if(mina.st[i][0]>0){
            dmin.st[i][0]=mina.st[i][0];
            xmax.st[i][0]=-0x3f3f3f3f3f3f3f3fll;
        }
    }
    for(int i=1;i<=m;i++){
        minb.st[i][0]=read();
    ...
    for(int i=1;i<=q;i++){
        ...
        if(minb._min_(l2,r2)>0){
            if(maxa._max_(l1,r1)>0){
                printf("%lld\n",maxa._max_(l1,r1)*minb._min_(l2,r2));
            }else{
                printf("%lld\n",maxa._max_(l1,r1)*maxb._max_(l2,r2));
            }
        }else if(maxb._max_(l2,r2)<0){
            if(mina._min_(l1,r1)>0){
                printf("%lld\n",mina._min_(l1,r1)*minb._min_(l2,r2));
            }else{
            ...
            if(a0[r1]!=a0[l1-1]){
                ...
                if(xmax._max_(l1,r1)!=-0x3f3f3f3f3f3f3f3fll){
                    ans=max(ans,xmax._max_(l1,r1)*maxb._max_(l2,r2));
                }
                if(dmin._min_(l1,r1)!=0x3f3f3f3f3f3f3f3fll){
                    ans=max(ans,dmin._min_(l1,r1)*minb._min_(l2,r2));
                }
